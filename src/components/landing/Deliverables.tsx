  ...
  return (
    <section id="entregaveis" className="bg-secondary py-16 md:py-24">
      <div ref={ref} className={`container mx-auto px-4 transition-all duration-700 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
        <h2 className="mb-4 text-center text-3xl font-bold text-secondary-foreground md:text-4xl">O que você recebe</h2>
        <p className="mx-auto mb-12 max-w-2xl text-center text-secondary-foreground/70">Cada avaliação inclui um pacote completo de documentos com validade técnica e jurídica.</p>
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {items.map(({ icon: Icon, title, description }, i) => (
            <div key={title} className={`rounded-xl border border-secondary-foreground/10 bg-secondary-foreground/5 p-6 transition-all duration-500 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`} style={{ transitionDelay: `${i * 100}ms` }}>
